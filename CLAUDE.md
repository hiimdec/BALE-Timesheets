...
**Fresh session?** Read HANDOVER.md for the current release state (2026.12 (12) live on the App Store since 10 September 2026; `main` and `develop` level at the close-out merge; one engine change on the web that the store build does not yet carry), the working disciplines in practice, and every parked item with context. MAINTENANCE.md holds the parked work; CALC_DECISIONS.md holds the calc rulings ledger.
## Build topology
