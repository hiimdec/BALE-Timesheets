...
**ADDENDUM (7 September 2026, from the 5 September device log; CORRECTED the same day):** the five mid-session `plugin.load` lines are app-process starts, and the content process never died (the TERMINATED line is in that build and absent from the log). They are NOT a fault: the founder's Analytics Data holds no crash or jetsam file for the 16:26-16:35 window, so the four unexplained deaths were ordinary reclaims of a suspended app while the share sheet and Files ran, relaunched by the next share. Not the dead-controls fingerprint. The one file that does exist, the 4 September watchdog kill, is its own record below. What the log still adds: a boot line after dead controls means the app was relaunched, not recovered - every native state resets with the process.
...
From the 5 September device log (iPhone 17, both reader paths): five `plugin.load | webview booted` lines in eight minutes, one per call-sheet import. They are app-process STARTS, not page reloads: `plugin.load` runs from `registerPluginInstance` inside `capacitorDidLoad`, once per bridge view controller; Capacitor's content-process handler only clears stored calls and listeners and reloads the document; and the build that wrote the log carries the always-on `webview.TERMINATED` line (same commit as the `nav.native` lines the log shows) and has none. Six imports ran across five processes; one ran warm and worked, so the reader does not need a fresh process. Each cold boot was a share-in launch (the new shoot mounts seconds after boot, which only a queued launch URL allows), and the process had died in the gap between leaving the app and sharing the next sheet: within 26, 5, 11 and 10 seconds of its last line. The process that did the last import lived on; the morning process lived six hours. Importing does not restart the process. CORRECTION (7 September 2026, founder's Analytics Data): there are NO crash or jetsam files for the 16:26-16:35 window. The four deaths were ordinary terminations - a suspended app reclaimed by iOS while the user fetched PDFs in Files with the share sheet running, relaunched by the next share. Not a fault, and the sentence that stood here ("something kills it when it is backgrounded") was wrong in its framing; so was the reading of them as the dead-controls fingerprint. The one file that exists is the 4 September 14:57 watchdog kill, recorded on its own below.

**Reader logging: the hold's condition is met.** The files answered: one watchdog kill, a blocked process at graceful termination. What that fault needs is in its own record below (the lifecycle line and the off-main moves), proposed, not built. The `app.exit` MetricKit subscriber and the footprint lines stay held.

**Built (small, independent, as listed):**
- The chooser has one door out: `closeSharedImport` clears the state, the phase and the dedupe ref together, like `closeShareLinkImport` beside it. The ref used to live for the whole process, so a second share of the same file path was silently ignored until a force quit: the founder's "sometimes the reader doesn't fire if I do multiple". Addition in the same spirit: a failed ingest clears the ref too, so a retry is a fresh share. With the 5 September deaths ruled ordinary (7 September), the dedupe ref is the most likely cause of "doesn't fire": it is the only candidate a force quit specifically fixes. One caveat only the device can settle: it bites only when iOS hands the same Inbox path twice, and before the Inbox clearance in this commit an undeleted earlier copy may have made iOS suffix the new one. A `share.url dedupe=hit` line is proposed with the watchdog record so the next occurrence names itself. A third, rarer shape exists: a share arriving while a New-shoot extraction is in flight is closed away when that extraction completes; its one-line fix is in the same proposal.
...
## ACCEPTED, NOT FIXED: the title's presence-verification exposure (7 September 2026)

Ruled 7 September 2026: leave the title alone. The reason, so this reads as a decision and not an oversight: the exposure is weak because the model title is never the first source. The label harvest runs first (title/production/client/project/job name/campaign labels), the masthead rule covers the label-less sheets, boilerplate is rejected, and a model title is the source only when neither fires and it is not boilerplate. On the 20-sheet corpus the pattern path names a title on every sheet (no empty title in the draft; 18 match the founder's file and two are short by a subtitle), so the model title fallback is not reached on any of them. A gate would need a vocabulary the title does not have: there is no payee phrase or label for "the title" beyond the labels the harvest already uses, so a gate would be the label harvest again. Revisit only if a device walk produces a wrong title from the model path; the fix shape would be the same context gate as the reference and the company.

## The prodCo context gate - BUILT (8 September 2026; proposed and measured 7 September)

**The measurement, and how it was taken.** FoundationModels is available on this Mac (macOS 26.6.2), so the pipeline's model path was mirrored in a scratchpad tool compiled against the tree's own CallSheetHarvest.swift: the same @Generable schema, instructions, greedy sampling, chunking, page plan, match-back, verification and pick, run over all 20 corpus sheets (1.5-4.6 s per page, 20 sheets in about ninety seconds), dumping every candidate with its line context; the gates were then judged offline against the founder's expected.txt. Caveat that matters: the Mac's model and the iPhone's diverge. On device (5 September) Nettwerk's company was right with Apple Intelligence on; the Mac's model names the page-1 client for it. The Mac numbers measure the SHAPE of the exposure, not the device's exact rate; the device runs are the truth.

**What the Mac's model does today, unguarded:** company right on 4 of 20, the same company in another form on 2, wrong on 14. Twelve of the fourteen are the page-1 brand, client or masthead (DFS, Nike, PepsiCo, UBS / Bloomberg, Epoch, McDonalds, Marks & Spencer, Audible, the Nettwerk client line, the Everlast brand line, Forever Living Products, the Umberto brand), verified by presence and winning by session order because page 1 runs first; two are glyph damage (Amahla's digit-glued name, InRehearsal's damaged layer). Nothing displaces any of them.

**The gate proposed:** a model company counts only when (a) its matched span sits on a line carrying a payee phrase (payeeVerbPattern) or a production-company label (prodCoLabelPattern), and (b) the value passes the harvest's own shape hygiene (plausibleCompanyValue, plus the payee stop phrases). A candidate that fails does not count at all, as the reference does not, so the pattern harvest fills, then the OCR fallback, then honest missing. Inside the gate a payee-line candidate ranks above a label-line candidate, the harvest's own order (Dove: the payee line carries the Limited form the founder's file expects; the page-1 label line carries the short form). Result on the 20: right on 16, wrong on 0, missing on 4 (Umberto: the payee is a person, a shape refused by design; Comet: the company is an image; TDA176: the ruled honest miss; Forever Living: see the fidelity gap below). Nothing that is right today goes wrong.

**Why the line and not the block** (the reference's shape): inside invoicing blocks the model returns junk that verifies by presence - an HMRC sentence's "Pulse Films" (TDA176), the addressee "F.A.O <person>" on the payee line itself (TDA176; the hygiene refuses it), the "address invoices to" fragment (Umberto), an email as a company (McDonalds), the glued-digit name (Amahla), the damaged-layer form (InRehearsal). A block-inclusive gate measures 13 right and 2 wrong; a label-within-three-lines gate admits Comet's client under a header row. A company is any capitalised phrase and blocks are full of them; a reference has one label vocabulary. Accepting a label alone on the line above (Brother's cell under a header) trades a right answer for the fuller legal form, so it is not accepted; the pattern's label-cell rule fills it in the file's form.

**TDA176 specifically:** today the Mac's model ships the brand line verified; the two other candidates are the addressee on the payee line (refused by the hygiene) and a company named in the HMRC paperwork sentence (refused by the line rule). Under the gate all three are refused, the pattern and the OCR fallback find nothing, and the field is missing: the ruled honest miss. The file says "Till Dawn" with the note that the company is an image; the only textual Till Dawn on the sheet is the AGENCY line, which no path returns; the record's ruling is that an agency is never the payee. The sheet stays red either way; whether the file should say (none) is the founder's call.

**A fidelity gap found on the way (Forever Living):** the harness's corpus mode reads the company from its OCR fallback with language correction OFF and top-left ordering; the plugin's OCR runs with language correction ON and mid-line bands. On this sheet the harness's settings find the label cell and the plugin's find nothing, so the harness's green is harness-only and the device would likely miss it. Its own small round: align the corpus mirror's OCR with the plugin's settings (the device is the truth), re-ratchet, and device-walk Forever Living.

**If ruled: pins.** PC cases executed through the harness (a payee line counts; a label line counts; a block-only company does not; an addressee on a payee line does not; a client label line does not; a masthead brand does not; a label alone above does not; a stop-phrase payee value does not; payee ranks above label), HS pins for the verify case, the loop drop and the rank, one mutation per clause. The pattern path and the phase-two ratchet are untouched by construction. The measurement tool could join the suite as an optional, loud-skip stage that runs only where the model is available; propose-first, since it is slow and model-version-dependent.

## The 4 September watchdog kill - a process that could not answer a graceful termination - PROPOSED, not built (7 September 2026)

**The file.** The only crash file on the founder's phone for the week: 4 September, launched 14:56:30, killed 14:57:07. `0x8BADF00D Failed to terminate gracefully after 5.0s`, `WatchdogEvent: process-exit`, `WatchdogVisibility: Background`, application CPU 0.000 s across the five seconds. Frames empty. Build stamped 2026.11 (11), which develop also stamps, so the tree is ambiguous. Reading: RunningBoard asked the process to exit while it was in the background; the graceful path delivers the request on the main thread (applicationWillTerminate, then exit) and gives five seconds; the app ran nothing in those seconds. Zero CPU is a wait, not work: a semaphore, a lock, a file-coordination barrier, a synchronous XPC round trip, or a process still frozen when asked. The frames being empty, the specific wait is not identified; what follows is every wait the code allows.

**What runs on backgrounding, named.** Native: AppDelegate's five lifecycle methods are empty; Capacitor's bridge posts a `pause` document event and the App plugin posts `appStateChange` (isActive false on willResignActive) and `pause` (didEnterBackground), both by async evaluateJavaScript; nothing native observes backgrounding otherwise. JS, on `appStateChange` inactive and `visibilitychange` hidden: the storage flush (DurableStore.write through the plugin queue: one atomic local file write, no lock, no fsync, resolves when the rename lands) and the iCloud sweep (once a day: status, writeSnapshot, listSnapshots, deleteSnapshot for the prune). Note that willResignActive also fires when the share sheet, the document picker or the switcher opens, so the day's snapshot can run while the app is still on screen.

**What can wait at zero CPU on the main thread.** Three, all synchronous system round trips: (1) the diagnostics ring's App Group UserDefaults read-modify-write in `TMLiveActivity.dbg` - cfprefsd XPC - on the main thread for every UIKit-side line (nav.native, chrome) and for `requestedAt` / `recordRequestedAt`; (2) ActivityKit in `startActivity`, dispatched to main: `Activity.activities`, `Activity.request`, `activityState` / `content` reads - XPC to the activities service; (3) WebKit's print pagination in NativePdf (`viewPrintFormatter`, `UIPrintPageRenderer.drawPage`) - a synchronous request to the content process, which a suspended content process cannot answer; export only. WebKit's own activity-state waits at backgrounding are bounded to a fraction of a second.

**What can wait at zero CPU off the main thread.** On the iCloud plugin's private queue: `url(forUbiquityContainerIdentifier:)` (documented to block), three `NSFileCoordinator.coordinate` barriers (write, read, delete; no timeout), the restore's 10 s download poll; on the bridge queue: `Activity.activities` in listActivities, `areActivitiesEnabled`, and every App Group UserDefaults access (events, active shoot, JS ring lines); the intent's drain hold (4 s), arm window (4 s) and undo window (5 s) are task suspensions, not thread waits. None of these holds the main thread, and exit does not wait for them.

**What nothing waits on.** The snapshot: JS awaits its promises; no delegate, scene or Capacitor handler blocks until it finishes; a coordination frozen mid-flight resumes with the process. The flush: the same; it cannot wait indefinitely, and a kill before the rename leaves the previous file (the 3 September kill test).

**Background time.** None requested anywhere: no beginBackgroundTask in the app or in Capacitor core, no UIBackgroundModes. So the flush, the snapshot and any Live Activity end run in the seconds before suspension or are frozen mid-flight; a process asked to exit while frozen, or while its main thread sits in one of the three round trips, produces exactly this file.

**Proposal (not built).**
1. No synchronous XPC on the main thread: the ring appends in memory and writes the App Group array from one serial diagnostics queue (event frequency kept; the per-line main-thread round trip and the 300-line rewrite leave the tap path); `startActivity`'s ActivityKit calls and the requestedAt map move to the same off-main serial queue (ActivityKit does not require the main thread); NativePdf stays as it is, export only, noted.
2. Bounded background work: on didEnterBackground the native side begins a background task with an expiration handler; JS's backgrounding work runs inside it and signals done; the task ends on that signal, on expiry, or at a fixed cap; the iCloud coordinate calls take the 15 s cancel timer already proposed and the container URL is cached after first success, so no coordination outlives the task.
3. One always-on ring line per lifecycle transition (`lifecycle.background`, `lifecycle.foreground`, `lifecycle.terminate`) carrying the count of plugin calls in flight, written from the diagnostics queue, so the next such file reads against what the app was doing.
4. Two share-in lines and one guard from the same investigation: `share.url` with `dedupe=hit|miss`; `closeSharedImport` after an extraction clears only if the chooser still holds the file that was extracted.
5. Pins: a structural audit that no `UserDefaults(suiteName:` call and no ActivityKit request sits on a main-thread path; executed pins for the ring queue's ordering and cap through a swiftc harness (App Group suites resolve on macOS); the lifecycle lines pinned at source; mutation per clause.

**Alternative reading, kept honest:** the process may already have been frozen when asked to exit, which is system-side. Items 1 and 2 hold either way; item 3 tells the two apart next time.

**BUILT (8 September 2026, founder-approved: "14 of 20 wrong is not a marginal call, and eight of those had the right answer in the patterns already, unable to win because of my byte-identity ruling. That ruling was right for regressions and wrong here").** `CallSheetHarvest.modelCompanyCounts(_:at:in:)` (payee or label line, company shape, no stop phrase) and `companyContextRank` (0 payee, 1 label, 2 neither; the pipeline form takes the key); `verify` gains the prodCo case; the candidate loop drops a failed company right after the failed-reference drop and carries the rank on the candidate; `pick` orders prodCo by rank between verified and session order. `resolveField` is untouched: a verified model value is still never displaced, the gate only narrows what verifies. The pattern path and the phase-two ratchet are unchanged by construction (9 = 9). Replaying the real model answers from the 7 September dump through the built rule against the founder's file: 16 right, 0 wrong, 4 missing (Umberto, Forever Living, Comet, TDA176), the measured result exactly. READ THAT FIGURE FOR WHAT IT IS: the corpus proves the forms it holds and nothing more. Twenty sheets held five company label forms, and the twenty-first sheet, a live job on 8 September, brought a sixth within an hour and shipped its label inside the company. Sixteen of twenty is a statement about these twenty sheets, not a general accuracy claim. Pins: PC1-PC10 and RK1-RK5 executed through the harness (three new kinds), HS22-HS24 structural; twelve mutations, each reddening its named clause; PC9's fixture was corrected before the battery so that its line qualifies and only the hygiene can refuse it. Xcode BUILD SUCCEEDED, zero warnings in our files; negative control failed at CallSheetHarvest.swift line 495; the restored tree built green.

**HELD, FLAGGED PROMINENTLY - the Forever Living fidelity gap.** The harness's corpus mode reads that sheet's company through its OCR fallback with language correction OFF and top-left ordering; the plugin's own OCR runs with correction ON and mid-line bands, and on this sheet it finds nothing. A green the device would not reproduce is worse than a red: it hides a miss behind a passing stage. Its own round: make the corpus mirror run the plugin's OCR settings (the device is the truth), re-ratchet (Forever Living's company goes red there until the plugin finds it), and device-walk the sheet. Until then, read the phase-two count as "green on the Mac's OCR settings".

**HELD - TDA176's expected line** is the founder's to settle (the file says Till Dawn, the record says an agency is never the payee, the only textual Till Dawn is the AGENCY line). The sheet is missing under the gate either way.

Device walks owed for this build: Gymshark and Nettwerk with Apple Intelligence on (company unchanged and right); a sheet whose page-1 brand the model used to return (DFS or Walkers if he has them to hand) now showing the payee-line company.

## Watchdog item 1 - no synchronous system call on the main thread - BUILT (8 September 2026)

Founder-approved as its own commit: "synchronous system calls on the main thread, on the path that handles every tap, is a defect independent of the watchdog". What moved, and where to:
- The diagnostics ring. `TMLiveActivity.dbg` takes the timestamp on the caller and does its App Group read-modify-write (a cfprefsd round trip) on one serial queue, `TMLiveActivity.diagQueue`. Order and content are unchanged: still a fresh read-modify-write against the shared suite per line, so a line written by the widget's process is never clobbered by a cached copy. The caller returns at once; the nav.native lines no longer cost the tap path a system round trip. The four accessors (set/get the flag, get/clear the log) run through the same queue, so a read issued after a line is ordered after it. `dbgFlush()` resumes once every queued line has landed; the three LiveActivityIntent perform() bodies await it before every return, so the suspension that follows an intent cannot eat a queued line.
- The Live Activity start path. `startActivity`'s block (the registry read, the request, the state reads, the requested-at map) dispatches to `laQueue`, a serial queue of the plugin's own, instead of main. ActivityKit does not require the main thread. The pre-existing cross-thread access to `currentActivity` (written in that block, read on the bridge queue) is unchanged by this move and noted.
- The chrome-state stamp in MainViewController writes on the diagnostics queue; the long-press export reads the ring on that queue and presents its sheet from main.
- Unchanged, noted: WebKit's print pagination in NativePdf (export only) and listSnapshots' NSMetadataQuery (run-loop driven, bounded, must be on main).

Pins: a new native-audit stage, `scripts/native-audit/main-thread.js`, MT1-MT6, structural (the two files import ActivityKit, AppIntents and Capacitor, which swiftc cannot compile off-device); nine mutations, each reddening its named pin. Five existing source pins reddened on the move and were RETARGETED BY NAME, each keeping every clause it held and gaining the new one: TT11d (the curtail no-op guard now flushes the ring before returning), DP3 (the export reads on the diagnostics queue and presents from main), DP5 and DX-S3 (the chrome stamp is taken on the caller and written on the queue; the five keys unchanged), DP7 (the native stage list gained main-thread.js). One mutation per retarget, each reddening its pin. Xcode BUILD SUCCEEDED with the two known deliberate deprecation warnings in TimeMachineAppShortcuts.swift and nothing from this change; negative control failed at TimeMachineIntents.swift line 145; the restored tree built green. Device walks owed: a burst of taps followed by a ring export reads in order with the taps' timestamps; a Live Activity starts from a fresh mount and adopts on relaunch; the diagnostics flag toggles and the log clears from Settings.

## Watchdog items 2, 3 and 4 - bounded background work, the lifecycle lines, the share-in lines - BUILT (8 September 2026)

**Item 2, the answer in plain sight.** Nothing in the app ever requested background time. From this commit the app holds ONE background task from didEnterBackground (`AppLifecyclePlugin`, co-located with the bridge VC like NativeChromePlugin, no new pbxproj entry). JS runs the storage flush and the day's iCloud snapshot inside it, through one coordinator on Capacitor's `pause` event (which IS didEnterBackground; the sweep used to fire on resign-active, which also fires for the share sheet and the picker with the app still on screen - it fires on true backgrounding now, once, re-entry guarded), and signals `backgroundWorkDone` with a one-line summary (`flush=ok snapshot=written ms=412`). The task ends exactly once on the first of: the signal, foreground, iOS's expiration handler, or a fixed 20 s cap. The iCloud plugin's three coordinations run under a 15 s cancel timer from a queue of their own (a cancelled coordination returns the cancellation error and rejects; the always-on `icloud.timeout` line names it), and the container URL is cached after the first success. The restore's 10 s download poll is unchanged (bounded).

**Item 3, the lines.** Always-on: `lifecycle.background` (task held or refused, remaining time), `lifecycle.background.done` (the JS summary), `lifecycle.background.end` (reason: done, expired or capped, and the hold in ms), `lifecycle.foreground`, `lifecycle.terminate`. The terminate line is the one place the app waits: half a second, bounded, at willTerminate only, for the queued line to land before the process exits - the only DispatchSemaphore in either target, pinned as such. Next time a watchdog file appears: a terminate line present means the main thread was free enough to take the request (so the block was elsewhere, or the wait was elsewhere); absent, the process was frozen or the thread parked before the request landed.

**Item 4, the share-in.** `share.url dedupe=hit|miss ext=...` on every arrival, so the next "didn't fire" names itself; and the extraction guard: the chooser closes after a New-shoot extraction only if it still holds the file that was extracted, so a share that arrived during the extraction keeps its chooser (`share.extract.superseded` traced).

**Pins.** `scripts/native-audit/lifecycle.js`, LC1-LC6 (structural; the files import Capacitor and UIKit), twelve mutations each reddening its pin. Storage: LJ1-LJ3 (the wrapper, the coordinator, the sweep's status words and the absence of its old listener), SR5-SR6 (the share lines and the guard), twelve mutations. Four existing sweep pins RETARGETED BY NAME, each keeping its clause on the new shape: IB3a (once a day), IB3b (never empty or mid-onboarding), IB3c (silent when unavailable; the reason now goes to the lifecycle line), IB3d (arms on true backgrounding through the coordinator, the ruled change); DP7 retargeted again for the lifecycle stage joining the gate. Xcode BUILD SUCCEEDED, zero warnings in our files; negative control failed at MainViewController.swift line 582; the restored tree built green.

**Device walks owed.** Background the app and export the ring: background, background.done with the summary, foreground, in that order, and the day's first backgrounding writes the snapshot; force-quit from the switcher while the app is fresh in the background: a terminate line; share the same sheet twice into a warm app: dedupe=miss then a second miss with the Inbox cleared, or a hit that names the earlier path; Settings' backup row still populates.

## REGRESSION on the first live sheet outside the corpus: the label survived into the company - BUILT (8 September 2026; proposed the same day)

**The finding.** A real call sheet from the founder's job that day, Apple Intelligence on, after 4510f55: the company came back as `COMPANY NAME DADBOD LTD`. The sheet is the first new sheet outside the 20 and it carries a label form the corpus does not contain: a `COMPANY NAME:` line. The corpus's only "company name" text is a bullet in HMRC boilerplate ("- company name" in a please-include list), never a label with a value; its company labels are PRODUCTION COMPANY, UK PRODUCTION COMPANY, US PRODUCTION COMPANY, PRODUCTION COMPANY INSURANCE and COMPANY ADDRESS.

**The mechanism, confirmed through the real harvest.** Before the gate the model's own span won. Now the model's span (`DADBOD LTD`, on the `COMPANY NAME:` line) fails the gate - no payee phrase, and `COMPANY NAME` is not in the label lexicon - so the pattern harvest fills, and the value comes from `companyNameWindow`: the suffix-anchored backward walk that trims `.,:;|` from every token before the capitalised-token test, so `NAME:` becomes `NAME`, the walk continues through the label, and the window is `COMPANY NAME DADBOD LTD`, which passes the shape hygiene. It fires on three routes: the suffix-in-block path (an invoicing block anchored by INVOICING ADDRESS, a section header or an HMRC line), and the label-cell walk under a PRODUCTION COMPANY header. A payee line carrying the label would leak the colon form instead (`COMPANY NAME: DADBOD LTD`); the founder's value had no colon, so it was the window. The label path proper (`prodco-label`) strips its own label by construction - it takes the text after the label match - which is why no corpus sheet shows the leak: every corpus label was in the lexicon and won before the window ran.

**Was a label boundary ever considered?** No. The window rule (9e74849, 1 September) was built from the first corpus run's junk captures: its glue list stops the walk at payee words ("ADDRESS TO MAD COW FILMS" yields the company) because those were the words the corpus put before a company; no corpus line put a label there. The address field gained `stripAddressLeadIn` on 4 September because a sheet showed the leak; the company had none because no sheet had shown it. The company is also the one field with no cleaner on the winner: the title has cleanTitle, the reference cleanRef, the address addressWithoutCompany.

**The sweep (the question that matters most).** Every pattern-path value on the 20 sheets, prodCo, reference and address first line, checked for a leading label word: none. The gate moved the winning source to the pattern on nine corpus sheets and all nine match the founder's file, so no masked defect surfaced on the corpus. The corpus is 20 sheets; the 21st found a form in an hour. A green corpus is evidence about the forms it contains and nothing else.

**The proposal (not built), three parts, each covering a route the others do not:**
1. The window walk stops at a label boundary: a raw token carrying a colon ends the walk before it is included. Alone, it fixes the three window routes (suffix-in-block, HMRC-anchored, the label cell); the gate still refuses the model's span, so the pattern wins with the right value.
2. `COMPANY NAME` joins the label lexicon. Alone, it fixes every route but the label cell (the label path catches the line first and strips its own label) and lets the model's span pass the gate (rank 1).
3. `cleanCompany`, the missing cleaner, run on whatever won: a leading label strips with or without a separator when it is a multi-word label (production company, company name, prod co) and only with a separator when it is a single word (company, name, payee, supplier, invoice to, bill to), so "Company Films Ltd" survives; a value that is only a label becomes missing. It alone fixes the payee-line form that carries the label with its colon.
Measured on the scratchpad copy: all six shapes give `DADBOD LTD`; the 20 corpus sheets are byte-identical on prodCo, reference and address; the model replay stays 16 right, 0 wrong, 4 missing, no winner touched by the cleaner. Pins if ruled: the window boundary, the lexicon form, the cleaner's edge cases executed through the harness; the cleaning wired in run() beside cleanRef (HS14's shape); one mutation per clause.

**Asked of the founder:** drop the sheet into ~/Developer/tm-callsheets with its expected block, so the 21st form is in the corpus and phase two asserts it; the ratchet then tightens or names it.

**BUILT (8 September 2026, founder-ruled: all three parts, since each covers a route the others do not, plus the reader's own field line; the last build before the release).** Part 1: `companyNameWindow`'s backward walk breaks on a raw token carrying a colon before the punctuation trim that used to erase it. Part 2: COMPANY NAME joins `prodCoLabelPattern`, so the label path reads the line and the model's own span on it passes the gate at rank 1. Part 3: `CallSheetHarvest.cleanCompany`, the missing sibling of the other three cleaners, runs in run() on whatever won, BEFORE the address dedupe (which compares against the settled company): a multi-word label strips with or without a separator, a single word only with one ("Company Films Ltd" survives), a letter lookahead keeps "Prod Company Ltd" whole, and a value that is only a label becomes missing. The reader's own lines: one always-on `reader.field` line per field naming state, source (model, model-fallback, harvest-email, harvest-label, harvest-masthead, pattern:<how>, ocr-fallback) and page, with the value for the company and the reference only, never an email or an address; every fill site tags its source and the payload carries it. Measured: all six shapes of the live form read DADBOD LTD; the 20 corpus sheets are byte-identical on company, reference and address (ratchet 9 = 9); the model replay through the tree's harvest with the cleaner applied stays 16 right, 0 wrong, 4 missing, no winner touched.

Pins: WB1-WB4 (the window boundary and its division of labour with the cleaner), CC1-CC12 (the cleaner's edges), PL1-PL5 (the live form through the whole harvest, the HMRC bullet refused, the payee form's raw colon), PC11 and RK6 (the gate and rank on a COMPANY NAME line), all executed; HS25-HS27 structural (the boundary and the lexicon, the cleaning order, the field lines and the source tags). Twenty-five mutations, each reddening its named clause; PL1 is covered by the two nets removed together, deliberately, since either alone still reads it. Two fixture corrections named: PL4's first strengthened form put an HMRC word within reach (a block anchor, so the suffix rule fired for its own reasons) and its second carried CATERING (a word the cell walk skips); the final form is a plain suffix line within the cell walk and outside any block, so the only route to a false company is the label path. Three existing pins retargeted by name, each keeping its clause: HS6 (the pattern entry now names its source), and S5 and UU1v (the label-title call now names its source; one mutation mislabelling it reddens both). Xcode BUILD SUCCEEDED, zero warnings in our files; negative control failed at CallSheetHarvest.swift line 512; the restored tree built green. Device walk owed: the live sheet again - the company reads DADBOD LTD, and the ring shows six reader.field lines with the company's source named. Nothing else is added before the release.

## DECISION: analytics is deliberately absent from the what's-new deck (9 September 2026)

Founder-ruled with the 2026.12 release copy: analytics has its own notice that asks properly, and listing it as a feature in the deck would read as burying it. It stays in the longer forms where someone is reading detail - RELEASE_HIGHLIGHTS (the Settings release notes) and CHANGELOG.md - and out of the deck. Pinned DK9: no page, kicker, headline, line or row of WHATS_NEW_PAGES may mention analytics, milestones, telemetry or the service; a mutation adding the word to a row reddens it. Nobody adds it later without moving that pin by name.

## The 2026.12 bump and release copy - BUILT (9 September 2026)

APP_VERSION and WHATS_NEW_VERSION to 2026.12 together (the gate fires only when they agree); TUTORIAL_VERSION stays 2 so the tutorial does not replace the what's-new for existing users. CURRENT_PROJECT_VERSION 12 and MARKETING_VERSION 2026.12 at all eight project lines, app and widget; Xcode resolves 2026.12 (12) for both targets. CHANGELOG gains its 2026.12 section; CLAUDE.md's release line corrected. home-preview.html stays at 2026.11 and moves on main when Apple approves; HANDOVER is redated at the release merge.

The deck: four pages, three heroes then the list, the founder's copy verbatim. The kickers are stored in sentence case and rendered upper by the component, so they read as written while the source keeps the house form. The list rows are separate title and line elements, so no separator is rendered; where a dash would appear in copy it is the spaced hyphen. Accents by the mockup's rule LC1 already pinned: amber for time (the lock screen), green for money (earnings; the crew rate fix, since it was costing money), sky for what travels with you (steps to a new phone), neutral for the one uncategorised row (Improvements). Icons from the existing set. WN1 executes the real gate from the source: both constants read 2026.12, a stored 2026.11 no longer matches, a fresh install fires too, dismissal turns it off. Three pins retargeted by name, each keeping its clause: Z9f (the 2026.12 literals), LC1 (the last row is Improvements), DK5 (the founder's "general fixes"). One mutation per clause, each reddening its pin.

## React #130 on launch after the 2026.12 copy - the deck's icon was an object, and no pin had ever rendered the deck - FIXED (9 September 2026)

**What broke.** The 2026.12 what's-new deck pointed its "Steps that stick around" row at `ICloudBackup`. That name is not an icon: it is the iCloud backup JavaScript wrapper, an object with an icon-shaped name that the icon inventory swept up with a `const I…` grep. React received an object where a component was expected (#130, "got: object", the render method of DeckListPage). The deck fires on launch for every user who saw 2026.11, the root boundary caught it, and "Go back" remounts Root, which fires the deck again: the founder was stuck on the boundary screen on that build. Records are untouched - the boundary only writes the breadcrumb. The way out was this fix build; nothing a user can change from the fallback alters the gate.

**Why the gate was green.** The storage sandbox stubs React, so it never evaluates WHATS_NEW_PAGES() as values; every deck pin was a source regex, and LC1's even accepts any icon-shaped NAME. The render smoke mounts Root through real react-dom, but its fixture keeps the tutorial due (seenTutorialVersion '99'), so whatsNewDue is false and the what's-new deck never mounted there. Nothing between the copy and the phone rendered the pages.

**Fixed.** The row's icon is `IUp`, a real component from the set. Two pins that would have caught it: WN2 (storage, executed through the exposed pages: all eight icons are functions, never an object or undefined) and R8 (render smoke: the real deck with the real pages mounted through real react-dom, every page in the track at once, every headline and row title in the DOM, exactly one svg per icon inside the page track, nothing thrown). One thing learned building R8: React 18 rethrows an unhandled render error outside the render call, so the first R8 crashed the whole stage under the device shape instead of going red; it now mounts under a boundary of the smoke's own with the process guarded for the window, and a bad icon is a named red. Three mutations - the device shape, an undefined hero icon, a row pointed at another object - each reddening WN2 and R8.

**Reported, not fixed (the founder's screenshot): the boundary's "Go back" button is not in the app's accent.** RootErrorBoundary renders ABOVE Root with hardcoded inline hex (background #0a0a0a, button #0ea5e9 on #000) and reads no theme token; the theme is a class the app toggles on the html element, and in poppy mode the accent is not the sky hex the button paints. Its own small round: paint the fallback from the tokens, or keep it deliberately theme-free and say so.

## Turnaround measured to the pre-call, TOC only - BUILT (10 September 2026; proposed and ruled the same day)

**The ruling.** The founder overruled the read-only report's literal reading of APA §5: for TOC only, the rest runs from the previous wrap to the person's earliest working moment on the day - the pre-call where one exists, otherwise the unit call. Lunch, CWD, overtime and day type keep measuring from the unit call. Recorded in CALC_DECISIONS.md ("Turnaround measured to the pre-call - TOC only") as a ruling that goes beyond the published terms, with the APA's own unit-call definition quoted and the reasoning, including the two confirmations given in the founder's words (the overnight pre-call E7, the department default B4).

**What changed.** One new pure helper, `tocDayStart(day)`, decides where the person's day starts, and it mirrors the pay block's three-way pre-call decision exactly: a pre-call before the call starts the day; an overnight pre-call of at most 12 hours starts it the evening before, as the pay block pays it; anything else is ignored, as the pay block ignores it. `restHoursBetween` reads the start from the helper; `calcTOC` returns two additive fields, `restTo` and `restToTime`, and the TOC line's detail names the pre-call when it ended the rest (`9.0h rest to 12:00 pre-call · 1.0h TOC`). Measured to the call the detail is byte-identical to before. The line object gains no key, the label is verbatim, the invoice's aggregated line items and the share text goldens are untouched. The pre-call explainer gained one sentence, the founder's wording, without the clause about the APA (ruled: that stays in the ledger).

**Nothing else moves, three legs.** The diff touches `tocDayStart`, `restHoursBetween`, `calcTOC`, the TOC line's detail and the explainer, and nothing inside `calculateDay`. Every pre-existing executed pin on lunch, CWD, overtime and day type stayed green with no retarget. TP12 holds the seam textually: the pay block never calls the rest measure, and the measure reads only call, wrap, the next-day flag, the date and the two pre-call fields.

**Measured before building.** A prototype of the measure ran against the real source engine: the ledger's figures (E1-E9), a sweep of 20,736 (call, pre-call) pairs on a ten-minute grid with zero disagreements between the pay block's pre-call decision and the measure's, and the Best Boy cases B1-B5 through the real `resolveDay`. The founder's backup of 1 September: 37 days, 2 with a pre-call, 0 days change, no sent invoice affected. The call-sheet corpus: the reader does not harvest a pre-call, and 2 of the 20 sheets mention one, so nothing flows from the corpus into this measure yet.

**Pins.** `scripts/build-vs-source-audit/calc-boundary-assertions.js`, stage TP, TP1-TP12 (27 clauses): the ledger's example and its control, the exact-10h boundary that pays but is not a breach, the half-hour, the 8 May after-the-call shape on a long and a short night, the overnight pre-call, the 2,304-pair sweep binding the measure to the pay block, the legacy truck-call alias, the Best Boy record-over-default cascade, a Day off, and the textual seam. Fourteen mutations, one per pin plus the detail and `restTo` clauses, each reddening its named pin, none silent, no harness crash, the baseline restored and sha-verified after each: M1 drop the before-call branch (TP1 and eleven others); M2 an empty pre-call starts at 00:00 (TP2, also A1j/A1k); M3 breach at exactly 10h (TP3); M4 the start loses its minutes (TP4, also A1j); M5 never ignore an after-call pre-call (TP5, TP6, TP8); M6 the call-measured result forgets which call (TP6a alone); M7 drop the overnight branch (TP7, TP8); M8 threshold 12 to 13 (TP8 alone); M9 drop the legacy alias (TP9, TP12c); M10 the call back into the sum (TP10 and twelve others); M11 a day with no call starts at 00:00 (TP11 alone, the Day off); M12 a lunch read in the measure (TP12b alone); M13 the detail without the pre-call (TP1d, TP3b, TP7b, TP10a); M14 `restTo` the wrong way round (TP1c and ten others).

**Found on the way, not fixed.** The engine's `notes` array, including "Pre-call appears to be after main call - ignored", is never rendered anywhere in the app: nothing reads `meta.notes`. A user whose pre-call sits after the call gets no on-screen signal that it was ignored, for pay and now for turnaround alike. Parked with the pre-call data entry item in HANDOVER.md; the proposal there is a visible line beside the pre-call field. Also noted: the day breakdown table hides every line's detail below the small breakpoint, so on the phone the row shows the label and the money; the invoice's day-by-day section, the timesheet print and the accountant export carry the detail.

**Device walk owed.** A night followed by a pre-call day: the breakdown row's detail on a wide screen, the invoice's day section reading "to HH:MM pre-call", the Best Boy grid's TOC chip on the member with the pre-call and not on the member without.

## 2026.12 close-out - App Store approved, web merged and verified live (10 September 2026)

Apple approved 2026.12 (12) and the listing went live on 10 September. The one item held for the approval, home-preview.html's softwareVersion, moved to 2026.12 (70048fd, gate GREEN). develop was merged into main as 3068d80 ("Merge branch 'develop' into main: 2026.12 close-out", parents 70048fd and e482492), gated GREEN on the merged tree, pushed at 06:49 UTC and verified live by fetching: the homepage serving softwareVersion 2026.12 and /app/assets/app.js byte-identical to the local publish set (sha 0b6b41bb, 1,797,726 bytes, tocDayStart present) - verified live at 06:50 UTC on 10 September, 39 seconds after the push, eight published files byte-identical, internal files still 404. The tag v2026.12 stays on 942ab0f. develop fast-forwarded to main; both counts 0. This record and the handover update were committed and pushed after the verification, a docs-only deploy with the published set byte-identical.

**What the merge carried beyond the store build.** e482492, turnaround measured to the pre-call, TOC only. App Store build 12 was archived at 751c7ba, before it, so the phone measures turnaround to the unit call until the next store build while the web measures it to the pre-call now. It belongs in the 2026.13 changelog section; the deck does not change for it.

**The pre-call data entry item re-framed (founder, 10 September).** The never-rendered "pre-call ignored" note is now the leading hypothesis for the pre-call that "would not save": it saved and was silently ignored. HANDOVER.md item 3 carries the test that decides it.

**Owed as post-release verification, from the records.** The first analytics event in the release bucket, the only proof the release path works, read as count() over the period; the TestFlight debug bucket, unprovable until a TestFlight build exists; the install marker across a real restore, fixture-proven only; the dead-controls fault, plausible fix shipped (c19e324, b221db5, 99886da), no occurrence since, unproven; the turnaround-to-the-pre-call walk, money, never on hardware; and the device walks the handover names: the reader walks (Gymshark with Apple Intelligence on, the live sheet's six reader.field lines), a burst of taps then a ring export in order, background then export showing the lifecycle lines with the summary, the same sheet shared twice into a warm app, Health access off with a counted day, the night-wrap edit, the kill-test day's two chips and banners, the trainee role, light mode on the iPhone 12, the Live Activity discard on midnight, Forever Living on the device.

## 2026.13 close-out - Capacitor 8.5 shipped in three stops, the version bump, no what's-new card (11 September 2026)

**The release.** Stop one `ee098c0` (8.3.4 to 8.5.1, the dependency alone), stop two `0b7597b` (UIScene by hand on the storyboard route, SC1-SC7), stop three with no commit of its own (the same code built under Xcode 27 RC 27A266a and walked on an iPhone 12 on iOS 27; `git status` clean afterwards, so Xcode 27 altered no tracked file). This round bumps APP_VERSION to 2026.13 (index.html, one line), MARKETING_VERSION to 2026.13 and CURRENT_PROJECT_VERSION to 13 (project.pbxproj, four lines each, app and widget). No CHANGELOG section and no deck entry (founder-ruled: nothing a user can see).

**The no-card ruling and what it costs.** WHATS_NEW_VERSION stays 2026.12 behind APP_VERSION 2026.13. The deck's gate is `WHATS_NEW_VERSION === APP_VERSION && seenWhatsNewVersion !== WHATS_NEW_VERSION`, so on 2026.13 the deck fires for nobody, including a user coming from 2026.11 who never saw the 2026.12 card, and the Settings button "Show what's new again" clears the edition and mounts nothing. Both put to the founder with the close-out. Two pins retargeted by name: Z9f's armed clause now asserts the ruled pair (edition 2026.12, app 2026.13); WN1 now asserts, executed from the source's own gate, that whatsNewDue is false for a user who saw 2026.11, for a fresh install and for a dismissed edition. One mutation (WHATS_NEW_VERSION moved to 2026.13 in a copy of the source) reddens both.

**Verified on hardware for the first time.** The pre-call turnaround (`e482492`): a 22:00 wrap, a 09:00 unit call with an 08:00 pre-call, the breach produced and the invoice PDF naming the pre-call as the measure. Off the unverified list.

**The pre-call entry fault, re-framed.** First-hand report on 2026.13 on the device: a number selected in the pre-call field, enter pressed, the value not going in, several attempts before it took. An input-layer symptom, not the engine ignoring a saved value; neither the founder nor the session could reproduce it afterwards. The hypothesis is now two, recorded in HANDOVER.md; parked for 2026.14.

**Added to the 2026.14 queue.** IA7 (red once on 11 September, green on rerun, tree unchanged - its wrap clause takes a timestamp the comparison does not strip); the Xcode 27 warnings (the renamed FoundationModels initialiser at CallSheetPlugin.swift:966, the weak-capture diagnostics).

**The derived data trap, both shapes**, is in the handover and in `0b7597b`'s message: stale packages in the GUI after a bump (Reset Package Caches, Clean Build Folder), stale build products across a toolchain switch (Clean Build Folder in both directions; a separate derived data path on the command line).
