...
## Crew editors — noOT is not copied by the CrewManager editor or the solo job-settings editor

Found during Phase 6's coefficient investigation; placed here durably in Phase 7 (previously tracked only in a session memory and an audit-pin comment). A live wrong-direction claim: it OVER-claims.

The card marks Director and Producer `noOT: true` (otCoef 0), and the calc reads `crew.noOT` to suppress overtime entirely. Two of the four crew-record writers carry the flag — the Best Boy commit (`...(form.noOT ? { noOT: true } : {})`, index.html ~14530) and QuickAddCrewSheet (seed + role change both ways, ~16050-16110). Two do not: the CrewManager editor's `onRoleChange` (~10990) and the solo job-settings editor's role change (~17800). A Director or Producer selected through either of those two gets `otCoef: 0` (the card copy) but no `noOT` flag — and the engine's read is `crew.noOT ? 0 : (Number(crew.otCoef) || 1)`, so the stored 0 falls through `|| 1` to **1.0×**: the member accrues weekday OT at 1T that the card says they should not have at all. The RW1 reads-have-writers pin and the S1 equivalence contract both note the asymmetry without blessing it (S1's triple deliberately excludes noOT).

**Trigger:** the next slice touching any crew editor, or the crew-write collapse slice (the six-writers item in PACT_BECTU_PLAN.md), whichever lands first.
**Change:** copy the flag in both editors the way QuickAddCrewSheet's role change does — set it when the card row carries it, delete it when it doesn't — OR fold all role-change writes into one shared helper (the collapse slice's job), which fixes it by construction. Then move S1's noOT note into the triple.
**Why parked:** it is a behaviour change on the money path (propose-first), the fix wants to ride the editor-collapse slice rather than add a seventh hand-maintained variant of the same write, and the affected roles (Director/Producer adding themselves through those two specific editors) are rare enough that the misdirection — over-claiming, visible on the breakdown — surfaces itself.
