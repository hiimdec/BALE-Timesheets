...
**Why parked:** the slice crosses the whole LA lifecycle and can only be finally verified on device across a real midnight. The full failure mode ships in the LIVE 5.4.0 build (the plugin, the qualifies-end branch, and husk dismissal are all on origin/main), so 2026.11 inherits rather than introduces it, and its prompt improves the overnight tail regardless.
