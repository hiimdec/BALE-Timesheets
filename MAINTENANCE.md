...
**Status (25 August 2026): the resolver slice is BUILT on develop** - `1a199af` (laShiftRecord + the prompt's CARD_LIFETIME_MS suppression grace), `14eb9e6` (the sweep's three lookups; the kill and the early linger dismissal close here), `a0bc751` (the descriptor + controller key; overnight re-mint and the Still-on-set restart close here). Pins NR1-NR14 + WP16-17, all negative-tested; TT7a/TT8b/TT10a moved WITH the ruling. Ownership and epochs are proven at synthetic midnights in the harness; the CARD across a real midnight is device-verify-pending (test plan in the session record).

**Still open, propose-first (awaiting ruling):** the ingest guard below - and one NEW finding from the same walk: the native intents stamp `ev.date` as UTC-today at PRESS time (TimeMachineAppShortcuts.swift, "matches appendEvent's date"), so a card press AFTER midnight on a still-running night shift is dated the new day and its apply targets the wrong record even when it drains immediately. The acceptance window and the apply target both need the resolver's notion of who owns the night.
